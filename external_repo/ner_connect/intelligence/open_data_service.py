...
ELEVATION_CACHE_FILE = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "data", "elevation_cache.json"))
...
def _elevation_key(pt) -> str:
    return f"{pt[0]:.4f},{pt[1]:.4f}"

def get_elevation_profile(coords_list: list, timeout: float = 8.0) -> list:
    """
    Real elevation (metres) from the Open-Meteo Elevation API for each coordinate.
    Successful lookups are cached on disk, so the same place always gets the same value.
    Points that could not be looked up come back as None - callers must not treat a
    ...
    missing = sorted({_elevation_key(pt) for pt in coords_list if _elevation_key(pt) not in cache})
    if missing:
        lats = ",".join(k.split(",")[0] for k in missing)
        lons = ",".join(k.split(",")[1] for k in missing)
        url = f"{OPEN_METEO_ELEVATION_URL}?latitude={lats}&longitude={lons}"
        for attempt in range(2):
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "NER-Connect Logistics/1.0"})
                with urllib.request.urlopen(req, timeout=timeout) as resp:
                    elevs = json.loads(resp.read().decode("utf-8")).get("elevation", [])
                if len(elevs) == len(missing):
                    fresh = {k: float(e) for k, e in zip(missing, elevs)}
                    with _elevation_lock:
                        try:
                            with open(ELEVATION_CACHE_FILE, "r", encoding="utf-8") as f:
                                stored = json.load(f)
    ...
    return [cache.get(_elevation_key(pt)) for pt in coords_list]
